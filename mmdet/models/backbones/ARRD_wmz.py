...
class ARRD_Multi_Level_FPN(BaseModule):
    def __init__(self, 
        in_channel=256,
        mid_channel=128,
        scale=0.4,
        ...
        super(ARRD_Multi_Level_FPN, self).__init__(init_cfg)
        '''
        ...
        if self.input_number >= 5:
            ...
            self.linear_c5 = MLP(input_dim=in_channel, embed_dim=in_channel*scale_linear*scale_linear)
    ...
    def forward(self, x):
        ...
        if self.input_number >= 5:
            _c5 = self.linear_c5(c5).permute(0,2,1).reshape(n, -1, c5.shape[2]*self.scale_linear, c5.shape[3]*self.scale_linear)
            _c5 = F.interpolate(_c5, size=(c1.shape[2]*self.scale_linear, c1.shape[3]*self.scale_linear),mode='bilinear',align_corners=False)        
        
        if self.input_number >= 4:
            _c4 = self.linear_c4(c4).permute(0,2,1).reshape(n, -1, c4.shape[2]*self.scale_linear, c4.shape[3]*self.scale_linear)
            _c4 = F.interpolate(_c4, size=(c1.shape[2]*self.scale_linear, c1.shape[3]*self.scale_linear),mode='bilinear',align_corners=False)

        if self.input_number >= 3:
            _c3 = self.linear_c3(c3).permute(0,2,1).reshape(n, -1, c3.shape[2]*self.scale_linear, c3.shape[3]*self.scale_linear)
            _c3 = F.interpolate(_c3, size=(c1.shape[2]*self.scale_linear, c1.shape[3]*self.scale_linear),mode='bilinear',align_corners=False)

        _c2 = self.linear_c2(c2).permute(0,2,1).reshape(n, -1, c2.shape[2]*self.scale_linear, c2.shape[3]*self.scale_linear)
        _c2 = F.interpolate(_c2, size=(c1.shape[2]*self.scale_linear, c1.shape[3]*self.scale_linear),mode='bilinear',align_corners=False)

        _c1 = self.linear_c1(c1).permute(0,2,1).reshape(n, -1, c1.shape[2]*self.scale_linear, c1.shape[3]*self.scale_linear)

        
        # Linear Fusion
        if self.input_number == 5:
            x = self.linear_fuse(torch.cat([_c1, _c2, _c3, _c4, _c5], dim=1))
...
@BACKBONES.register_module()
class ARRD_Single_Level(nn.Module):
    def __init__(self, scale_factor=2, in_c=256, out_c=3, ps_rate=4):
        super(ARRD_Single_Level, self).__init__()
    ...
    def forward(self, x):
        x1 = self.conv_init(x)
        add = F.interpolate(x1, scale_factor=self.scale_factor, mode='bilinear')
        x1 = self.res_1(x1)
        x1 = self.res_2(x1)
...
@BACKBONES.register_module()
class ARRD_Multi_Level_FPN_CONV(BaseModule):
    def __init__(self, 
        in_channel=256,
        mid_channel=128,
        scale=0.4,
        ...
        #embedding_dim=1024, 
        mode='bilinear',
        input_number=5, # Number of level features use for reconstruction
        init_cfg=dict(type='Kaiming',
                     layer='Conv2d',
                     a=math.sqrt(5),
                     distribution='uniform',
                     mode='fan_in',
                     nonlinearity='leaky_relu')):
        super(ARRD_Multi_Level_FPN_CONV, self).__init__(init_cfg)
        '''
        mode (str) – algorithm used for upsampling: 'nearest' | 'linear' | 'bilinear' 
        | 'bicubic' | 'trilinear' | 'area'. Default: 'bilinear'
        '''
        ...
        self.ps = nn.PixelShuffle(self.scale_pixel)
        self.conv = conv_block(in_channel, in_channel*self.scale_linear*self.scale_linear, 1, 1, 0, 'c')
    def forward(self, x):
        ...
        if self.input_number >= 5 and not begin:
            _c5 = self.conv(c5).reshape(n, -1, c5.shape[2]*self.scale_linear, c5.shape[3]*self.scale_linear)
            _c5 = F.interpolate(_c5, size=(c1.shape[2]*self.scale_linear, c1.shape[3]*self.scale_linear),mode='bilinear',align_corners=False)        
        
        if self.input_number >= 4:
            _c4 = self.conv(c4).reshape(n, -1, c4.shape[2]*self.scale_linear, c4.shape[3]*self.scale_linear)
            _c4 = F.interpolate(_c4, size=(c1.shape[2]*self.scale_linear, c1.shape[3]*self.scale_linear),mode='bilinear',align_corners=False)

        if self.input_number >= 3:
            _c3 = self.conv(c3).reshape(n, -1, c3.shape[2]*self.scale_linear, c3.shape[3]*self.scale_linear)
            _c3 = F.interpolate(_c3, size=(c1.shape[2]*self.scale_linear, c1.shape[3]*self.scale_linear),mode='bilinear',align_corners=False)

        _c2 = self.conv(c2).reshape(n, -1, c2.shape[2]*self.scale_linear, c2.shape[3]*self.scale_linear)
        _c2 = F.interpolate(_c2, size=(c1.shape[2]*self.scale_linear, c1.shape[3]*self.scale_linear),mode='bilinear',align_corners=False)

        _c1 = self.conv(c1).reshape(n, -1, c1.shape[2]*self.scale_linear, c1.shape[3]*self.scale_linear)
